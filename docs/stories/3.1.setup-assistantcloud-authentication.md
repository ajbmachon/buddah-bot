...
- [ ] Task 1: Create AssistantCloud account and project (AC: 1, 2)
  - [ ] Navigate to https://cloud.assistant-ui.com
  - [ ] Sign up for free account
  - [ ] Create new project named `buddahbot`
  - [ ] Note the Frontend API URL from dashboard
  - [ ] Generate and save API key

- [ ] Task 2: Configure local environment variables (AC: 3, 6)
  - [ ] Add to `.env.local`:
    - `NEXT_PUBLIC_ASSISTANT_BASE_URL=<frontend_api_url>`
    - `ASSISTANT_API_KEY=<api_key>`
  - [ ] Update `.env.local.example` with new variable templates
  - [ ] Add comments explaining each variable's purpose
  - [ ] Verify `.env.local` is in `.gitignore`

- [ ] Task 3: Install AssistantCloud dependencies (AC: 3)
  - [ ] Run: `npm install @assistant-ui/react@latest`
  - [ ] Run: `npm install @assistant-ui/react-ai-sdk@latest`
  - [ ] Verify package.json shows latest versions
  - [ ] Run `npm install` to ensure all dependencies resolve

- [ ] Task 4: Create basic connection test (AC: 5)
  - [ ] Create test file: `lib/assistantcloud-test.ts`
  - [ ] Import AssistantCloud class
  - [ ] Create instance with anonymous mode
  - [ ] Test basic operations (initialize)
  - [ ] Run test in development: `npm run dev` and check console
  - [ ] Verify no connection errors

- [ ] Task 5: Deploy environment variables to Vercel (AC: 7)
  - [ ] Navigate to Vercel Dashboard → Project → Settings → Environment Variables
  - [ ] Add `NEXT_PUBLIC_ASSISTANT_BASE_URL` (Production scope)
  - [ ] Add `ASSISTANT_API_KEY` (Production scope)
  - [ ] Verify variables are saved
  - [ ] Redeploy production to pick up new env vars

- [ ] Task 6: Document AssistantCloud setup
  - [ ] Update README with AssistantCloud setup instructions
  - [ ] Document anonymous mode decision rationale
  - [ ] Add troubleshooting section for common issues
  - [ ] Link to AssistantCloud documentation
...
**Why AssistantCloud for BuddahBot:**
...
### Anonymous Mode vs Authenticated Mode

**Anonymous Mode (MVP Choice):**
```typescript
...
**Use Case:** MVP with friends/family, no account system needed
**User ID:** Generated per browser session, stored in localStorage
**Threads:** Persisted per browser, not shared across devices
**Perfect for:** Testing, prototypes, single-user apps

**Authenticated Mode (Post-MVP):**
```typescript
...
const cloud = new AssistantCloud({
  baseUrl: process.env.NEXT_PUBLIC_ASSISTANT_BASE_URL!,
  authToken: async () => session?.user?.id,
});
```

**Use Case:** Production apps with user accounts
**User ID:** From Auth.js/NextAuth session
**Threads:** Synced across devices per user
**Migration Path:** Simple - just swap anonymous for authToken
...
- API key not needed for anonymous mode but good to have for future
- AssistantCloud uses project-level auth, not user-level in anonymous mode
